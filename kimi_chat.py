...
class KimiChat(Plugin):
    def __init__(self):
        ...
        try:
            ...
            self.analyze_image_type = conf.get("analyze_image_type", "paddleocr")
            ...
            logger.error("[KimiChat] 初始化失败, 没有获取到config.json配置文件。")
            raise
        except ValueError as e:
            logger.error(f"[KimiChat] 初始化失败，错误: {e}")
            raise
        except Exception as e:
            logger.error("[KimiChat] 初始化失败。" + str(e))
            raise
    ...
    def _handle_image_context(self, user_id, content, msg):
        ...
        if user_id in self.params_cache:
            logger.warn(f"[KimiChat] 开始处理图片解析！")
            msg.prepare()
            # 提取prompt的值
            ...
            if prompt:
                ...
                image_information = analyze_image_paddle(content) if self.analyze_image_type == "paddleocr" else analyze_image(content, self.azure_api_url, self.azure_api_key)
                ...
                if not image_information:
                    return "非常抱歉，识别图片出了点问题，您可以再试试，还是不行请通知我的主人修一修。"
